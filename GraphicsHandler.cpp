...
void GraphicsHandler::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
        ...
        if(result != VK_SUCCESS) { G_EXCEPT("Failed to allocate command buffer for data transfer"); }
        ...
        if(result != VK_SUCCESS) { G_EXCEPT("Failed to begin command recording"); }
        ...
        if(result != VK_SUCCESS) { G_EXCEPT("Failed to end command recording"); }
        ...
        if(result != VK_SUCCESS) { G_EXCEPT("Failed to submit for staging buffer transfer"); }

        result = vkQueueWaitIdle(m_GraphicsQueue);
        if(result != VK_SUCCESS) { G_EXCEPT("Queue wait failed"); }
